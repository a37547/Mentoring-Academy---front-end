import React from "react";
import { Row, Col, ListGroup, Jumbotron } from "react-bootstrap";

const AssociacoesEstudantes = () => {
  return (
    <Jumbotron>
      <Row className="d-flex justify-content-center">
        <Col xs={12} sm={11}>
          <h2>Associações Estudantis</h2>
          <p>
            As associações estudantis organizam diversas atividades académicas,
            culturais e de integração dos novos alunos que permitem fomentar o
            espirito académico e o desenvolvimento intelectual dos seus
            estudantes.
          </p>
          <p>
            A Associação Académica do IPB, está organizada em associações de
            estudantes em cada uma das escolas que constituem o IPB e cada
            escola possuí Núcleos de Estudantes organizados por curso.
          </p>
          <p>
            Podes contribuir de forma ativa para o futuro do IPB, ao
            participares nas diversas atividades promovidas pelas Associações
            Estudantis do IPB.
          </p>
          <ListGroup className="my-4">
            <ListGroup.Item>
              <h5>Associação Académica do IPB(Aaipb)</h5>
              <p>Email: aaipbraganca@gmail.com</p>
              <p>
                Página no Facebook:{" "}
                <a
                  href="https://www.facebook.com/aaipb/?epa=SEARCH_BOX"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  https://www.facebook.com/aaipb/?epa=SEARCH_BOX
                </a>
              </p>
            </ListGroup.Item>
          </ListGroup>
          <ListGroup className="my-4">
            <ListGroup className="my-4">
              <ListGroup.Item>
                <h5>Associações de estudantes das escolas do IPB</h5>
                <ListGroup className="my-4">
                  <ListGroup.Item>
                    <h5>ESAB</h5>
                    <p>Email: aeesabraganca@hotmail.com</p>
                    <p>
                      Página no Facebook:{" "}
                      <a
                        href="https://www.facebook.com/AEESABOFFICIAL/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        https://www.facebook.com/AEESABOFFICIAL/
                      </a>
                    </p>
                  </ListGroup.Item>
                </ListGroup>
                <ListGroup className="my-4">
                  <ListGroup.Item>
                    <h5>ESTIG</h5>
                    <p>Email: aeestig.alunos@gmail.com</p>
                    <p>
                      Página no Facebook:{" "}
                      <a
                        href="https://www.facebook.com/aeestigipb/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        https://www.facebook.com/aeestigipb/
                      </a>
                    </p>
                  </ListGroup.Item>
                </ListGroup>
                <ListGroup className="my-4">
                  <ListGroup.Item>
                    <h5>ESSa</h5>
                    <p>Email: associacao.saude.braganca@gmail.com</p>
                    <p>
                      Página no Facebook:{" "}
                      <a
                        href="https://www.facebook.com/AE-ESSa-IPB-1034340679918739/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        https://www.facebook.com/AE-ESSa-IPB-1034340679918739/
                      </a>{" "}
                    </p>
                  </ListGroup.Item>
                </ListGroup>

                <ListGroup className="my-4">
                  <ListGroup.Item>
                    <h5>ESEB</h5>
                    <p>Email: Aeeseb2019@hotmail.com</p>
                    <p>
                      Página no Facebook:{" "}
                      <a
                        href="https://www.facebook.com/ae.eseb/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        https://www.facebook.com/ae.eseb/
                      </a>
                    </p>
                  </ListGroup.Item>
                </ListGroup>
                <ListGroup className="my-4">
                  <ListGroup.Item>
                    <h5>ESACT</h5>
                    <p>Email: geral@aeesact.pt</p>
                    <p>
                      Página no Facebook:{" "}
                      <a
                        href="https://www.facebook.com/AeEsactIpb/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        https://www.facebook.com/AeEsactIpb/
                      </a>
                    </p>
                  </ListGroup.Item>
                </ListGroup>
              </ListGroup.Item>
            </ListGroup>
          </ListGroup>
          <h4>Associativismo Internacional do IPB</h4>
          <ListGroup className="my-4">
            <ListGroup.Item>
              <h5>Associação dos Estudantes Africanos em Bragança - AEAB</h5>
              <p>
                <a
                  href="https://www.facebook.com/Associação-de-Estudantes-Africanos-em-Bragança-792518937438511/?__tn__=%2Cd%2CP-R&amp;eid=ARCPLpL_9sRQsDNjPP71oUMH9E5akVokHr-xQ60Rww3yJ1IsVYbneQZGgQ-C8Ct7ZVpt4_k-FDLAY_t1"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  https://www.facebook.com/Associação-de-Estudantes-Africanos-em-Bragança-792518937438511
                </a>
              </p>
              <p>
                <a
                  href="https://www.facebook.com/teamaeab/"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  https://www.facebook.com/teamaeab/
                </a>
              </p>
            </ListGroup.Item>
          </ListGroup>
          <ListGroup className="my-4">
            <ListGroup.Item>
              <h5>Associação dos Estudantes Brasileiros do IPB - AEBIPB</h5>
              <a
                href="https://www.facebook.com/aebipb/?__tn__=KH-R&amp;eid=ARClG3VEob3EICWL84ZB6K9ucDBtL8fktsfwfbJ4nUoPRtz25nWJKW36uUon1K-xZpkFbg8h2a4iYrqy&amp;fref=mentions"
                target="_blank"
                rel="noopener noreferrer"
              >
                https://www.facebook.com/aebipb/
              </a>
            </ListGroup.Item>
          </ListGroup>
          <ListGroup className="my-4">
            <ListGroup.Item>
              <h5>Núcleo da ESN em Bragança</h5>
              Erasmus Student Network Bragança - ESN Bragança:{" "}
              <a
                href="</a></li>
"
                target="_blank"
                rel="noopener noreferrer"
              >
                https://www.facebook.com/ESNBraganca/
              </a>
            </ListGroup.Item>
          </ListGroup>
          <ListGroup>
            <ListGroup.Item>
              <h5>Tunas e grupos culturais</h5>
              <p>
                No IPB existem um conjunto de Tunas e grupos culturais nos quais
                todos os estudantes podem participar.
              </p>
              <h5>Bragança</h5>
              <ListGroup className="my-4">
                <ListGroup.Item>
                  <h6>- Real Tuna Universitária de Bragança, RTUB.</h6>
                  <p>
                    <a
                      href="http://rtub.alunos.ipb.pt/"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      http://rtub.alunos.ipb.pt/
                    </a>
                  </p>
                  <p>
                    <a
                      href="https://www.facebook.com/rtub.tuna.braganca"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      https://www.facebook.com/rtub.tuna.braganca
                    </a>
                  </p>
                </ListGroup.Item>
              </ListGroup>

              <ListGroup className="my-4">
                <ListGroup.Item>
                  <h6>- Tuna Feminina do IPB, Tôna Tuna</h6>
                  <p>
                    <a
                      href="http://www.tonatuna.alunos.ipb.pt/"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      http://www.tonatuna.alunos.ipb.pt/
                    </a>
                  </p>
                  <p>
                    <a
                      href="https://www.facebook.com/TonaTunaIPB/"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      https://www.facebook.com/TonaTunaIPB/
                    </a>
                  </p>
                </ListGroup.Item>
              </ListGroup>
              <ListGroup className="my-4">
                <ListGroup.Item>
                  <h6>- Tuna Mista de Bragança, Rauss Tuna</h6>
                  <p>
                    <a
                      href="https://www.facebook.com/rausstunabraganca/"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      https://www.facebook.com/rausstunabraganca/
                    </a>
                  </p>
                </ListGroup.Item>
              </ListGroup>
              <ListGroup className="my-4">
                <ListGroup.Item>
                  <h6>- Grupo AfroSamba do IPB</h6>
                  <p>
                    <a
                      href="https://www.facebook.com/UlissesCorreiaSilva/photos/basw.AbpKTHodV0ONtQqP_7mWVsY-0fwfBpqhLJn0fNfFMjB6MI0saQXLQPrY1E0zXXxP1hkuOosrnRFHytbY2Sr03HWoVc342p6w7qNgYR1xezVAQxeDawtF9YGpIO3H4Xp43YpPDAtVz-EKCDkw_4Y_pri5.2367463819944549.1766713640035119.1766713530035130.1766713383368478.1766713386701811/1766713386701811/?type=1&amp;theater"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      https://www.facebook.com/UlissesCorreiaSilva/
                    </a>
                  </p>
                </ListGroup.Item>
              </ListGroup>
            </ListGroup.Item>
            <ListGroup.Item>
              <h5>Mirandela</h5>
              <ListGroup>
                <ListGroup className="my-4">
                  <ListGroup.Item>
                    <h6>- Tuna Masculina de Mirandela, In Vinus Tuna.</h6>
                    <p>
                      <a
                        href="https://www.facebook.com/In-Vinus-Tuna-933396293420235/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        https://www.facebook.com/In-Vinus-Tuna-933396293420235/
                      </a>
                    </p>
                  </ListGroup.Item>
                </ListGroup>
                <ListGroup className="my-4">
                  <ListGroup.Item>
                    <h6>- Tuna Feminina da ESAcT, Tuna Mira.</h6>
                    <p>
                      <a
                        href="https://www.facebook.com/tunamiraesact/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        https://www.facebook.com/tunamiraesact/
                      </a>
                    </p>
                  </ListGroup.Item>
                </ListGroup>
              </ListGroup>
            </ListGroup.Item>
          </ListGroup>
        </Col>
      </Row>
    </Jumbotron>
  );
};

export default AssociacoesEstudantes;
